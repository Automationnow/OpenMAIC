...
- Button text should reflect what will happen when clicked:
  - "Start" / "Begin" → Start simulation
  - "Pause" → Pause running simulation
  - "Resume" / "Continue" → Resume paused simulation
  - "Restart" / "Reset" → Reset and start fresh (when ended)
...
**When the user clicks "Start", there MUST be OBVIOUS visual animation. ALL button text MUST be in English.**
